...
```{r ans11}
as.integer(20*10*2+20*15*2+10*15*2) ->surface_A
 surface_A
```
...
單號請用1表示、雙號則用0表示。
```{r ans21}
str_extract(Mask[[1]][[1]],"[0-9]$") ->a21
as.integer(a21) %% 2 -> `1st_odd_or_even`
 `1st_odd_or_even`
```
...
odd_or_even <- function(id){
  a22 <- str_extract(id,"[0-9]$")
  as.integer(a22) %% 2 -> aa22
  return(aa22)
}
...
get_courses <-function(entranceYear,major){
  pickyear1 <- courseStructure$`學年`== as.integer(entranceYear)
  pickyear2 <- courseStructure$`學年`== as.integer(entranceYear)+1
  pickyear3 <- courseStructure$`學年`== as.integer(entranceYear)+2
  pickyear4 <- courseStructure$`學年`== as.integer(entranceYear)+3
  
  pickmajor1 <- str_detect(courseStructure$`應修系級` , paste0(major,"1"))
  pickmajor2 <- str_detect(courseStructure$`應修系級` , paste0(major,"2"))
  pickmajor3 <- str_detect(courseStructure$`應修系級` , paste0(major,"3"))
  pickmajor4 <- str_detect(courseStructure$`應修系級` , paste0(major,"4"))
  
  pick1 <- pickyear1 & pickmajor1
  pick2 <- pickyear2 & pickmajor2
  pick3 <- pickyear3 & pickmajor3
  pick4 <- pickyear4 & pickmajor4
  
  a31 <- list(
    year1=unique(courseStructure$`科目名稱`[pick1]),
  year2=unique(courseStructure$`科目名稱`[pick2]),
  year3=unique(courseStructure$`科目名稱`[pick3]),
  year4=unique(courseStructure$`科目名稱`[pick4])
  )
  return(a31)
}
...
entranceYear=100:103,
year1= map(get_coursesAllYears(entranceYear,"經濟系"),~.x[[1]]),
year2= map(get_coursesAllYears(entranceYear,"經濟系"),~.x[[2]]),
year3= map(get_coursesAllYears(entranceYear,"經濟系"),~.x[[3]]),
year4= map(get_coursesAllYears(entranceYear,"經濟系"),~.x[[4]])
