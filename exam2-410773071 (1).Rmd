...
```{r ans21}
library(stringr)
str_extract(data228$`檔號`,"[:upper:][0-9]+[:upper:]") -> channame
str_extract(data228$`檔號`,"(?<=/)00[0-9][0-9](?=/)") -> datayears
data228$`全宗名編號` <- factor(channame)
data228$`年` <- as.integer(datayears)
...
```{r ans22}
pick_22 <- data228$`附註` == "表冊名稱:臺灣省二二八事變正法及死亡人犯名冊"
data228$`個人名稱`[pick_22] -> victims
 victims
```
...
str_detect(data228$`個人名稱` ,"(吳金練|吳斐如|李瑞峰|李瑞漠|阮朝日|林介|邱金山|施江南|涂光明|張宗仁|許錫謙|陳忻|陳能通|陳澄波|曾鳳鳴|湯德章|黃何純|黃媽典|葉秋木|廖進井|潘木枝|盧鈉欽|蘇憲章)"
) -> aa
table(data228$`檔號`[aa]) ->hh
names(hh) <-victims
hh
...
factor_class <- factor(courseStructure2$`應修系級`)
level_class <- levels(factor_class)
str_split(level_class,"、",n=50) ->courseStructure2
str_extract(courseStructure2 ,"[:graph:]+系[0-9]")
